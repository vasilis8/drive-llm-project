...
\begin{enumerate}
    ...
    \item \textbf{Collision Penalty:} Immediate reward of $-15$
        (category-dependent: $-5$ for aggressive, $-25$ for
        conservative), terminating further reward computation.
\end{enumerate}
...
\toprule
\textbf{Weight} & \textbf{Default} & \textbf{Aggressive} & \textbf{Conservative} & \textbf{Defensive} & \textbf{Neutral} \\
\midrule
$w_s$ (speed)              & 3.0   & \textbf{5.0}   & \textbf{1.5}   & \textbf{2.0}   & 3.0 \\
$w_m$ (smoothness)         & 0.1   & \textbf{0.05}  & \textbf{0.5}   & 0.1            & 0.1 \\
$w_l$ (lane dev.)          & 0.1   & 0.1            & 0.1            & \textbf{0.5}   & 0.1 \\
$w_b$ (speed bonus)        & 2.0   & \textbf{4.0}   & \textbf{1.0}   & \textbf{1.5}   & 2.0 \\
$r_{\text{crash}}$         & $-15$ & \textbf{$-5$}  & \textbf{$-25$} & \textbf{$-20$} & $-15$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Conservative:} Low speed weight ($1.5$), high
        smoothness penalty ($0.5$), harsh collision penalty ($-25$) —
        rewards smooth, cautious driving with strong crash avoidance.
