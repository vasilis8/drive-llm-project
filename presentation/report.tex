...
progressively introduces four command categories—neutral, aggressive,
defensive, and conservative—over the course of 2 million timesteps. We
...
\begin{enumerate}
    \item \textbf{CARLA Simulation Environment:} Provides the racing
        ...
        detection.
    \item \textbf{Command Manager:} Maintains a vocabulary of 24
        natural language commands across 4 categories, handles
...
We define a vocabulary of \textbf{24 natural language commands} organized
into four behavioural categories, each with 6 semantically varied
...
\addlinespace
\textbf{Conservative} & 6 & ``Conserve tires'', ``Save energy and drive
smoothly'', ``Cruise at a steady pace'', ``Easy on the brakes'',
``Smooth steering inputs only'', ``No unnecessary risks'' \\
\addlinespace
...
24 commands at initialisation time and cached for efficient lookup
during training.
...
\begin{itemize}
    ...
    \item \textbf{Throttle/Brake:} $v \in [-1, 1]$ — Negative values apply braking
\end{itemize}
...
language-conditioned behaviour. It consists of five terms, each weighted
by base values that can be \textbf{overridden per command category}:
...
r_t = \begin{cases}
    ...
    w_m \cdot r_{\text{smooth}} - w_l \cdot r_{\text{lane}} & \text{otherwise}
\end{cases}
\end{equation}
...
\begin{enumerate}
    \item \textbf{Progress Reward} ($r_{\text{progress}}$): Rewards
        ...
        \]
        where $d_t$ is the cumulative distance traveled.
    ...
    \item \textbf{Collision Penalty:} Immediate reward of $-100$,
        terminating further reward computation.
\end{enumerate}
...
shown in \textbf{bold}; unmodified weights use default values.}
\label{tab:reward_modifiers}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Weight} & \textbf{Default} & \textbf{Aggressive} & \textbf{Conservative} & \textbf{Defensive} \\
\midrule
$w_s$ (speed)         & 0.5 & \textbf{1.5} & \textbf{0.2} & \textbf{0.3} \\
$w_m$ (smoothness)    & 0.3 & \textbf{0.1} & \textbf{1.0} & 0.3 \\
$w_l$ (lane dev.)     & 0.2 & 0.2          & 0.2          & \textbf{0.8} \\
$w_p$ (progress)      & 1.0 & 1.0          & 1.0          & 1.0 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{Aggressive:} High speed weight (1.5$\times$), minimal
        smoothness penalty — encourages fast, dynamic driving.
    \item \textbf{Conservative:} Low speed weight (0.2$\times$), high
        smoothness penalty (1.0) — rewards smooth, energy-efficient
        driving.
    \item \textbf{Defensive:} High lane deviation weight (0.8) — strongly
        penalises leaving the optimal racing line, encouraging position
        holding.
...
For each command category $c \in \{\text{aggressive, conservative,
defensive, neutral}\}$, we measure the following quantities across $N$
evaluation episodes:
...
\begin{enumerate}
    ...
    \item \textbf{Smoothness Index} ($\mathcal{M}_c$): Mean steering
        ...
        \]
        \emph{Expected:} Conservative $<$ Neutral $<$ Defensive $<$
        Aggressive.
    ...
    \item \textbf{Lane Adherence} ($\mathcal{L}_c$): Mean lane
        ...
        \mathcal{L}_c = \frac{1}{N} \sum_{i=1}^{N}
            ...
            |d_{\text{lane}}^{(i)}(t)|
        \]
        \emph{Expected:} Defensive $<$ Neutral $<$ Conservative $<$
        Aggressive.
    ...
    \item \textbf{Collision Rate} ($\mathcal{C}_c$): Fraction of
        ...
        \]
        \emph{Expected:} Conservative $<$ Defensive $<$ Neutral $<$
        Aggressive.
...
\midrule
Speed Ratio ($\mathcal{S}$) & Aggressive $>$ Neutral $>$ Defensive $>$ Conservative \\
Smoothness ($\mathcal{M}$) & Conservative $<$ Neutral $<$ Defensive $<$ Aggressive \\
Lane Adherence ($\mathcal{L}$) & Defensive $<$ Neutral $<$ Conservative $<$ Aggressive \\
Collision Rate ($\mathcal{C}$) & Conservative $<$ Defensive $<$ Neutral $<$ Aggressive \\
Survival Length ($\mathcal{T}$) & Conservative $>$ Defensive $>$ Neutral $>$ Aggressive \\
\bottomrule
\end{tabular}
\end{table}
...
command categories} during training. Presenting all four command types
...
Phase 3 & 400K – 2M   & All four categories \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{Phase 1 (Neutral):} The agent first learns basic
        driving: track following, collision avoidance, and speed
        management. This provides a stable foundation.
    \item \textbf{Phase 2 (+Aggressive):} The most behaviourally
        contrasting category is introduced, forcing the agent to begin
        differentiating its policy based on command embeddings.
    \item \textbf{Phase 3 (All):} Conservative and defensive categories
        are added simultaneously, completing the full command space.
        The agent has sufficient policy flexibility from Phase 2 to
        absorb these related but distinct strategies.
\end{itemize}
...
Training Speed      & $\sim$255 it/s \\
Total Training Time & $\sim$2.2 hours \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Metric} & \textbf{Aggressive} & \textbf{Conservative} & \textbf{Defensive} & \textbf{Neutral} \\
\midrule
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Compliance Metric} & \textbf{Aggressive} & \textbf{Conservative} & \textbf{Defensive} & \textbf{Neutral} \\
\midrule
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \fbox{\parbox{\textwidth}{\centering\vspace{3cm}
        \textbf{[PLACEHOLDER: conservative/mean\_reward]}\\
        TensorBoard screenshot
        \vspace{3cm}}}
        \caption{Conservative reward over training}
    \end{subfigure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        Agent driving with conservative command
        \vspace{4cm}}}
        \caption{Conservative driving behaviour}
    \end{subfigure}
...
strategies for aggressive, conservative, defensive, and neutral
commands.

The curriculum learning approach—introducing command categories
